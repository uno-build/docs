...
`Image` displays a registered image resource. It does not download the image from `src`. Follow [Images](./images.mdx) to load and register the asset before calling `vue.render({})`.
