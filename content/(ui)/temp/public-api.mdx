...
`remove()` destroys a child; `detach()` preserves it for reinsertion. A text node cannot contain child nodes. See [Core API](./core-api.mdx) for lifecycle and ownership details.
...
`StyleName` is the union of supported style names. `StyleProps`, `NodeHandle`, `ScrollViewHandle`, and `InputHandle` are separately re-exported by `uno-ui/react`, `uno-ui/solid`, and `uno-ui/vue`. See [Styles](../api/styles.mdx) for the complete property and value reference.
