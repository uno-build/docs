...
Use one registered root per UI and call `solid.render(props)` once to mount it. Drive subsequent updates with signals; another `render()` call creates a new root instead of updating the existing root's props.

Call `solid.unmount()` to remove the component tree and run its cleanup before destroying the UI. It does not destroy the UI or dispose shared resources; see [Destroy](../getting-started/destroy.mdx).
...
In Solid, refs on `View`, `Text`, and `Image` receive the Uno node directly: use `node.layout` or `node.focus()`, without `nodes.main`. `ScrollView` and `Input` instead expose compound handles. Import `ScrollViewHandle` and `InputHandle` from `@uno/ui/solid` when using TypeScript.
...
{/* 
Use callback refs to store the handles. Clicking the view below focuses the input and scrolls the viewport:

```tsx
import { Input, ScrollView, View, useUI } from '@uno/ui/solid'
import type { InputHandle, ScrollViewHandle } from '@uno/ui/solid'

function Example() {
  const ui = useUI()
  let input: InputHandle | undefined
  let scroll: ScrollViewHandle | undefined

  function focusAndScroll() {
    input?.focus()
    if (scroll) scroll.nodes.main.scrollTop = 40
    ui.update()
  }

  return (
    <ScrollView ref={(handle) => { scroll = handle }} style={{ height: '120px' }}>
      <Input ref={(handle) => { input = handle }} value="Hello" />
      <View onClick={focusAndScroll}
        style={{ height: '300px', flexShrink: '0', backgroundColor: '#4377bb' }} />
    </ScrollView>
  )
}
```

Set `scrollTop` or `scrollLeft` on `nodes.main`, then call `ui.update()`. Use refs only while mounted and prefer props for styles and text. See [Node](../api/node.mdx) for the full API. */}
...
<iframe
  src="https://codesandbox.io/embed/x5dj2k?view=preview&module=%2Fcomponents%2FSolid.jsx&initialpath=%2Fsolid.html&hidenavigation=1&hidedevtools=1"
  style={{
    width: '100%',
    height: '350px',
    border: 0,
