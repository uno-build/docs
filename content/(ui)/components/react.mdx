...
Use one registered root per UI. Call `react.render(props)` again to update the root component's props while preserving its state.

Call `react.unmount()` to remove the component tree and run its cleanup before destroying the UI. It does not destroy the UI or dispose shared resources; see [Destroy](../getting-started/destroy.mdx).
...
Pass a React ref to access a component's Uno nodes. These refs expose handles rather than DOM elements. Import `NodeHandle`, `ScrollViewHandle`, and `InputHandle` from `@uno/ui/react` when using TypeScript.
...
import { Input, ScrollView, View, useUI } from '@uno/ui/react'
import type { InputHandle, ScrollViewHandle } from '@uno/ui/react'

function Example() {
  const ui = useUI()
  const input = useRef<InputHandle | null>(null)
  const scroll = useRef<ScrollViewHandle | null>(null)

  useEffect(() => {
    input.current?.focus()
    if (scroll.current) scroll.current.nodes.main.scrollTop = 40
    ui.update()
  }, [ui])
...
<iframe
  src="https://codesandbox.io/embed/x5dj2k?view=preview&module=%2Fcomponents%2FReact.jsx&initialpath=%2Freact.html&hidenavigation=1&hidedevtools=1"
  style={{
    width: '100%',
    height: '350px',
    border: 0,
