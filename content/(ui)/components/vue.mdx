...
Vue updates call `ui.update()` automatically after rendering changes. Keep the viewport setup and render loop from [First Layout](../getting-started/first-layout.mdx) to draw the UI with `ui.draw()`.

Use one registered root per UI. Call `vue.render(props)` again to update the root component's props while preserving its state.

Call `vue.unmount()` to remove the component tree and run its cleanup before destroying the UI. It does not destroy the UI or dispose shared resources; see [Destroy](../getting-started/destroy.mdx).
...
`Image` displays a registered image resource. It does not download the image from `src`. Follow [Images](../getting-started/images.mdx) to load and register the asset before calling `vue.render({})`.
...
Use a Vue template ref to access a component's Uno nodes. These refs expose handles rather than DOM elements. Import `NodeHandle`, `ScrollViewHandle`, and `InputHandle` from `@uno/ui/vue` when using TypeScript. Use `shallowRef` to hold a handle without deeply wrapping its nodes in reactive proxies.
...
{/* 
This example focuses the input and scrolls the viewport after mounting:

```vue
<script setup lang="ts">
import { onMounted, shallowRef } from 'vue'
import { Input, ScrollView, View, useUI } from '@uno/ui/vue'
import type { InputHandle, ScrollViewHandle } from '@uno/ui/vue'

const ui = useUI()
const input = shallowRef<InputHandle | null>(null)
const scroll = shallowRef<ScrollViewHandle | null>(null)

onMounted(() => {
  input.value?.focus()
  if (scroll.value) scroll.value.nodes.main.scrollTop = 40
  ui.update()
})
</script>

<template>
  <ScrollView ref="scroll" :style="{ height: '120px' }">
    <Input ref="input" value="Hello" />
    <View :style="{ height: '300px', flexShrink: '0' }" />
  </ScrollView>
</template>
```
...
<iframe
  src="https://codesandbox.io/embed/x5dj2k?view=preview&module=%2Fcomponents%2FVue.vue&initialpath=%2Fvue.html&hidenavigation=1&hidedevtools=1"
  style={{
    width: '100%',
    height: '350px',
    border: 0,
