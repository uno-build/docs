...
UI for rendering to a WebGPU canvas. Import from `@uno/ui/UI`. For engine surfaces, see [UIWorldSpace](./ui-world-space.mdx).
...
| `resources` | `ResourcesWebGPU \| null` | [Resource store](./resources-webgpu.mdx). |
...
| `create()` | `Node \| undefined` | Creates a detached [Node](./node.mdx). |
...
`register_platform_events` registers canvas listeners during creation. `UIOptions` and `DefinedEvent` are exported by `@uno/ui/UI`. See [Custom event definitions](./events.mdx#custom-event-definitions) for a `defined_events` example.
