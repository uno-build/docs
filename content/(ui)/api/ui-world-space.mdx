...
Shared reference for [UIThree](#uithree), [UIBabylon](#uibabylon), [UIBabylonLite](#uibabylonlite), and [UIPlayCanvas](#uiplaycanvas). These classes share the [UI instance API](./ui.mdx#methods), with the additions and overrides below. `UIWorldSpace` is their abstract base and has no public import path.
...
`WorldSpaceDrawOptions` accepts only `submit?` and `command_encoder?` from [WebGPU draw options](./ui.mdx#draw-options).

## Additional creation options

Engine factories accept [UI creation options](./ui.mdx#creation-options) plus:
