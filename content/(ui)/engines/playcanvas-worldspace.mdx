...
`ui.destroy()` releases Uno's offscreen texture and UI state. Remove the plane and release its engine resources as described in [Destroy: PlayCanvas](../getting-started/destroy.mdx#playcanvas). Dispose shared resources only after the last UI using them has been destroyed.
