...
For pointer, click, and wheel events, the payload described in [Event data](../getting-started/interactivity.mdx#event-data) also includes `distance_to_camera` when the pointer hits the plane. It is the distance from the camera to the hit position in Babylon Lite world units. The `x` and `y` fields remain coordinates in the UI's logical coordinate space.
...
`ui.destroy()` releases Uno's offscreen texture and UI state. Remove the plane and release its engine resources as described in [Destroy: Babylon Lite](../getting-started/destroy.mdx#babylon-lite). Dispose shared resources only after the last UI using them has been destroyed.
